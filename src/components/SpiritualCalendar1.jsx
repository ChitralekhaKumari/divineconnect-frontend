...
// Fixes: dates shifting back by 1 day, and missing dots on the calendar grid
...
// Takes the raw date value from the backend (could be "2026-06-26" or
// "2026-06-26T00:00:00.000Z") and returns just the "YYYY-MM-DD" part.
// Never passes the value through `new Date(string)` for comparison —
// that step is what causes timezone day-shifting bugs.
function dateOnly(rawDate) {
  if (!rawDate) return '';
  return String(rawDate).slice(0, 10);
}

// Formats a "YYYY-MM-DD" string for display, building the Date from
// local y/m/d numbers instead of parsing the string (which is parsed
// as UTC and can shift the displayed day backward by one).
function formatDateStr(rawDate) {
  const key = dateOnly(rawDate);
  if (!key) return '';
  const [y, m, d] = key.split('-').map(Number);
  const safe = new Date(y, m - 1, d);
  return safe.toLocaleString('default', { month: 'short', day: 'numeric', year: 'numeric' });
}
...
export default function SpiritualCalendar() {
  ...
  // ── Fetch festivals for the visible month (for dots on the grid) ──────────
  useEffect(() => {
    setLoading(true);
  ...
  // ── Fetch events for the selected day (detail panel under the grid) ───────
  useEffect(() => {
  ...
  // Compares using plain "YYYY-MM-DD" strings only — no Date parsing.
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border transition-colors ${fastOnly ? 'bg-amber-100 border-amber-300 text-amber-800' : 'bg-stone-50 border-stone-200 text-stone-500'
                }`}
            >
              <Flame size={12} />
          ...
          <div className="mt-6 pt-5 border-t border-stone-100">
            ...
            {dayEvents.length === 0 ? (
              <p className="text-sm text-stone-400">No events on this date.</p>
            ) : (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2 mt-2">
                        {(f.tags || []).map(tag => (
                          <span
                            key={tag}
                            className={`text-[11px] px-2.5 py-1 rounded-full font-medium ${tag === 'Fasting Day' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-700'
                              }`}
                          >
                            {tag}
                          </span>
